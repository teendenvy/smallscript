...
if [ "$1" != "--help" ]; then
	...
	if [ ! -f "/usr/local/nginx/conf/vhost/$configname.conf" ]; then
	echo "==========================="
	echo "domain=$domain"
	echo "===========================" 
	else
	echo "==========================="
	echo "$configname is exist!"
	echo "==========================="	
	fi
	...
	echo "Please input the listen port(default 80):" 
	read port
	if [ "$port" = "" ]; then
		port=80
	fi
...
cat >/usr/local/nginx/conf/vhost/$configname.conf<<eof
server
	{
		listen $port;
		...
		location /
			{
				...
				if (!-e \$request_filename) {
					...
					rewrite ^/(?!index\.php|images|style|js|system|download)(.*)$ /index.php/$1 last;
					break;
				}
			}
		location ~ .*\.(php|php5)?$
			{
				...
				fastcgi_param  SCRIPT_FILENAME $vhostdir/index.php;
				try_files \$uri =404;
